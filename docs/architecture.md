...
Threads can remain in memory for disposable sessions or use the durable SQLite implementation. SQLite appends allocate the sequence and write the event in one transaction, apply versioned migrations, redact payloads before persistence, and support fail-closed idempotency keys. Immutable side-table checkpoints commit to complete contiguous event prefixes with a canonical SHA-256 chain. The CLI can verify those prefixes and publish deterministic redacted artifacts atomically with owner-only permissions and no replacement; artifact verification remains inert and offline. A read-only retention planner requires a matching persisted checkpoint and protected artifact, then reports exact removable and retained ranges without mutating history. Append-only prepared anchors let suffix verification continue from an accepted prefix digest, while minimal idempotency tombstones prevent compacted keys from becoming reusable. Preparing an anchor preserves every event and the authoritative next sequence; actual deletion remains deferred to a later atomic compaction command. See [ADR 0005](adr/0005-durable-sqlite-threads.md), [ADR 0010](adr/0010-thread-checkpoints-and-protected-export.md), [ADR 0011](adr/0011-checkpoint-anchored-thread-retention.md), and [ADR 0012](adr/0012-prepared-retention-anchors.md).
